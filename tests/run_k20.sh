#!/usr/bin/env bash
# Standalone K20 unit test (no CTest infra in this project; follows the
# "standalone g++ on the real sources" precedent of tests/run_k18.sh).
# The kernel_info module is pure C++ (no Qt, no alpm, no fmt — std-only),
# so this compiles the real source with the project's GCC warning set and
# runs the 15-field extraction assertions against injected
# KernelInfoProbes only.
#
# Recipe shape (one forced deviation from k18's single driver command,
# the k19 one): the two TUs are compiled separately with the k18 flag set
# (incl. -O3 -flto -fwhole-program), then linked with LTO WITHOUT the -W
# flags — exactly the canonical cmake pipeline shape. A single command
# would carry the -W flags onto the link-time LTRANS whole-program pass,
# where GCC 16's -Wnull-dereference re-analysis flags a libstdc++
# istreambuf_iterator false positive in src/kernel_info.cpp's
# read_small_file (4 header warnings, zero project-code warnings) — the
# harness's real-probe smoke makes the module's probes live (zcat +
# sysctl + lsmod + modinfo + llvm-objdump, all read-only), while the
# cmake build leaves them dead (no app caller until chunk 2) and its
# link line carries no -W flags, so the cmake build stays 0-warning.
#
# Gates:
...
#      read-only — the real-probe smoke runs zcat/sysctl/lsmod/modinfo/
#      llvm-objdump without root; the md5 gate proves it, the k13/k14
#      pattern)
set -euo pipefail
cd "$(dirname "$0")/.."

OUT="/tmp/km-test-k20"
LOG="/tmp/km-test-k20-build.log"
...
/usr/bin/c++ $FLAGS -Isrc -c tests/test_k20_kernel_info.cpp -o "$OUT.t.o" 2> "$LOG" \
    || { echo "error: k20 compile of the test TU failed:" >&2; cat "$LOG" >&2; exit 1; }
/usr/bin/c++ $FLAGS -Isrc -c src/kernel_info.cpp -o "$OUT.d.o" 2>> "$LOG" \
    || { echo "error: k20 compile of the module TU failed:" >&2; cat "$LOG" >&2; exit 1; }
/usr/bin/c++ -O3 -flto -fuse-linker-plugin -fPIE "$OUT.t.o" "$OUT.d.o" -o "$OUT" 2>> "$LOG" \
    || { echo "error: k20 LTO link failed:" >&2; cat "$LOG" >&2; exit 1; }
...
if [[ "$(grep 'warning:' "$LOG" | grep -vc 'lto-wrapper:' || true)" -gt 0 ]]; then
    echo "error: k20 build produced source warnings:" >&2
...
if [[ -n "$ETC_MD5_BEFORE" && "$ETC_MD5_BEFORE" != "$ETC_MD5_AFTER" ]]; then
    echo "error: the k20 harness changed /etc/pacman.conf (before=$ETC_MD5_BEFORE after=$ETC_MD5_AFTER)" >&2
    exit 1
fi
echo "K20: OK (${OUT})"
