...
# the real binaries are never touched (k13B sandbox-PATH pattern).
#
# Gates:
#   0.  bash -n src/build_helper.sh (syntax)
#   A.  "unknown public key <40-hex>" (libalpm) — v1.23.0 regression guard:
#       1 recv-keys (exact 40-hex) + 1 ownertrust + exactly 2 makepkg calls + rc 0
...
run_case() { # run_case <id> <fail-text line...>
    ...
    set +e
    PATH="$BIN:$PATH" K16_SANDBOX="$SANDBOX" bash "$HELPER" -sfC "$SANDBOX/pkg" \
        > "$SANDBOX/out.log" 2>&1
    RC=$?
    set -e
}
